...
用 registry 和 thread 消息协调派发、scope、冲突和合并。代码/Git 是实现事实源，registry 是协作事实源；无高收益不触发。
...
BRT 首次写入前调用 `preflight`；`CLEAR/PEERS_NO_OVERLAP` 返回原 owner，`OVERLAP` 才协调。核对 thread 后运行 `status/join`。只 claim 最小 scope；仅在 checkpoint、blocker、pause/resume、merge-ready 和完成时 `update`。

owner 顺序：用户指定 > 有效 claim/registry > 更早 owner。非 owner 停止自身冲突写入，不默认要求既有 owner 暂停；争议面只读并回 BRT。
...
owner 保留关键路径与集成，继续不冲突工作。结果、拒绝或超时后释放派发 claim；超时派发标 stale，迟到结果先复核再采用。可选协作不 BLOCKED。简单任务、同文件或忙碌 Agent 不派发。
...
4. 对方重读 registry/Git/claim，执行 `resume` 重查重叠，再回复 `RESUMED`。`resume` 会关闭已清债 coordination；不要随后调用 registry `respond --status RESUMED`。
