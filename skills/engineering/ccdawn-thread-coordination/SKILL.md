...
需共同确定边界、顺序或接口时为 `DISCUSSION_REQUIRED`：复用 open coordination，否则 `open --kind discussion` 并发送一次 `DISCUSSION_REQUEST`。双方各给 position，coordinator 决定并广播；期间继续安全工作。
...
讨论默认优先于暂停；同一 participants + surface 存在则复用，不重复发送。
