...
5. 内部检查需求覆盖、正确性、范围、测试、安全/数据、维护性和集成影响，只输出有 finding 或真实证据缺口的视角。
...
- `P3 NICE_TO_HAVE`：不阻塞的清理、命名或文档。

每条包含紧凑文件/行号或 diff 位置、问题、影响、建议方向和验证条件。没有问题时明确“未发现阻塞性问题”，并说明尚未覆盖的证据边界。
