...
只 claim 最小 scope。pause 让出 claim，resolve 形成 `resumePendingAgentIds`；owner 失活由 `takeover` 接管，恢复重查重叠，明确取消/归档才可 `cancel-resume`。open coordination 或恢复债务阻止 complete。`agent_work_guard.py` 仅为兼容入口。
