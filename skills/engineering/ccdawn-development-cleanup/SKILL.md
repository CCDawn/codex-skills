...
- 存在临时产物、任务专用缓存、scratch 文件、claim、feature branch 或 worktree 时加载本 skill；完全干净时由当前 owner 记为 `NOOP`，不展开流程。
...
无候选时只说 `清理检查: NOOP`。有动作时仅汇报：已清理、保留/延后、验证证据和一个下一步建议；不打印完整文件树、全部分支或命令日志。
