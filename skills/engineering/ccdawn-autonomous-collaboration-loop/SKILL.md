...
- `Recovery Dispatcher`：任何活跃参与者在 Loop Owner 租约过期时可 `takeover`，继承未发送消息、暂停恢复和集成义务。
...
每个 peer 用 `MERGE_READY` 提交 `Base / Head or Artifact / Changed Scope / Tests / Dependencies / Risks`。Integration Owner 以 Git 和新鲜测试为事实源：
...
- 每个 peer 的原任务已完成或由用户明确取消，不以“消息已发送”代替完成；
