...
- UI/UX 方向、信息层级或视觉决策：`ccdawn-ui-design`；契约明确后的生产前端实现：`ccdawn-frontend-engineering`；已有界面或截图审查：`ccdawn-ui-review`。
...
当前 owner 用风险相称的新鲜证据收口。普通任务无需 `ccdawn-completion-summary`；只有跨阶段/会话、恢复、正式交接或 Deferred 风险才使用。代码写入后静默检查清理资格：无候选记 `NOOP`，有真实残留才路由 cleanup。
