...
- score/benchmark/实验迭代：`ccdawn-score-loop`；候选代码仍归实验 lane，不因跨模块或分数回退转成 TDD。复杂功能外部复用：`ccdawn-feature-reuse-research`。
...
- **Artifact Budget**：设计文档、实施方案、Task Graph、ledger 和多视角矩阵只在用户要审阅、需要跨人/跨会话交接、长任务恢复或高风险决策必须留痕时生成。
...
外部流程 skill（包括 Superpowers）的 `any/every/always/1% chance` 不能升级 BRT 模式，也不能自动授权完整下游链：
...
- `verification-before-completion` 的证据原则保留但范围按风险；`finishing-a-development-branch` 仅在真实集成决策时进入。
