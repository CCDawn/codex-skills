...
- 简单任务默认走轻量实现和必要验证；不要为了流程完整强制 BDD/TDD。
- BDD/TDD 只用于模型判断无法一次稳定完成、容易偏离、跨模块或高风险的功能。
- 直接实现可用于用户明确选择、单点小改、可逆、可验证且不改变核心行为的任务。
- 改代码前必须完成必要对齐；除非用户明确要求跳过。

## 流程路由

BRT 是 CCDawn 流程入口，只负责需求对齐和实现前闸门。需求对齐完成后先判定执行模式，再询问用户是否进入下一步。

执行模式：

- FAST_PATH：需求清楚、单点低风险、模型判断可一次完成；轻量实现 + 必要验证，不进入完整流程，不使用 BDD/TDD。
- COMPACT_FLOW：同一主题有多个小任务；使用一个连续工作区/上下文，简化任务列表，只有复杂子任务升级 BDD/TDD。
- FULL_FLOW：模型判断无法一次稳定完成、跨模块、高风险、迁移/权限/数据/发布相关，或用户要求严格流程；进入标准流程。
...
- 验证锚点：最低但足够的验证方式；只有升级到 FULL_FLOW 或复杂子任务时才要求 BDD/TDD 锚点；
...
A. 轻量实现（简单任务推荐）...
B. 进入 ccdawn-planning 制定实施方案（复杂任务推荐）...
C. 继续需求对齐...
D. 暂停...
```
