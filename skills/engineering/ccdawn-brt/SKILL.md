...
效率闸门为 `FAST / CHECK / PROFILE`：普通任务 `FAST`；明确 N+1、循环 I/O、重复全算或无界增长留给 owner `CHECK`，不因“发现问题”转 bug。仅故障、正确性回归或根因不明交 `ccdawn-bug-review`；性能目标/回归或热路径、规模、并发、资源风险才 `PROFILE`，交 `ccdawn-performance-engineering`。不为每次开发建立 benchmark。
