...
动作规则：`DISCUSS_FIRST 触发 -> 先讨论再路由`；执行型任务先过显式对齐闸门：`HIGH + SILENT_EXECUTION_ALLOWED -> act`、`HIGH/MEDIUM + FAST_PATH -> MICRO/ALIGN 后执行`、`COMPACT_FLOW/FULL_FLOW -> 对齐后给短计划或路由 planning`、`LOW -> ask or reversible probe`、`BLOCKED -> ask one blocking question`。
...
- COMPACT_FLOW：同一主题有多个推进单元；先输出 2-5 步短计划或路由判断，再复用当前工作区和上下文，必要时进入 `ccdawn-task-splitting` 判定拆不拆。
- FULL_FLOW：需要方案、拆分判定、跨模块、高风险、迁移/权限/数据/发布相关，或用户要求严格流程；必须路由 `ccdawn-planning`，不直接实现。
