...
BRT 默认适配每句输入：理解意图、选 owner、控制重量并推进到验证。用户无需输入 `/brt`。

简单任务直接完成；高影响误解才展示对齐。约束用于防错，不替代模型推理。
...
- 已对齐的多个现有平级会话需要持续协商、各自完成任务并共同集成：`ccdawn-multi-agent-orchestration`；单次建议、冲突、暂停恢复或合并协商：`ccdawn-thread-coordination`；真实开发残留：`ccdawn-development-cleanup`。
...
发现阶段不发消息、不提出协作、不暂停；没有正收益候选时静默回原 owner。BRT 不参与后续会话讨论或合并。
...
效率闸门为 `FAST / CHECK / PROFILE`：普通功能 `FAST`；循环 I/O、重复全算、隐藏高阶或无界增长由当前 owner 静默 `CHECK` 并修；明确性能目标/回归或需测量的高频、大规模、并发、缓存、批处理、流式、包体/启动风险才 `PROFILE`，路由 `ccdawn-performance-engineering`。不为每次开发建立 benchmark。
