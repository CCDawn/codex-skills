...
- 直接实现可用于用户明确选择、单点小改、可逆、可验证且不改变核心行为的任务。
- 改代码前必须完成必要对齐；除非用户明确要求跳过。

## 流程路由

BRT 是 CCDawn 流程入口，只负责需求对齐和实现前闸门。需求对齐完成后先判定执行模式，再询问用户是否进入下一步。不要为了流程完整强迫进入规划、任务拆分或 BDD/TDD。
...
- 需要评价、评估、审查合理性、比较方案、判断流程是否繁琐：路由到 `ccdawn-evaluation`。
- 用户报告 bug、回归、失败测试、异常行为、复现不了或“为什么会这样”：路由到 `ccdawn-bug-review`。
...
- 只是评价或审查：路由到 `ccdawn-evaluation`、`ccdawn-bug-review` 或 `ccdawn-pr-review`，不进入开发流程。
- 新 worktree 只在并行、冲突、高风险隔离或用户明确要求时使用。

阶段交接规则：

- 阶段完成后默认给下一动作选择；如果用户已授权连续推进或当前是 FAST_PATH，可推进到下一个自然闸门再汇报。
...
3. 主动揣测：给出 2-3 个候选意图，说明各自适合什么情况和你推荐哪一个。
4. 自评流程重量：判断是否 `FAST_PATH / COMPACT_FLOW / FULL_FLOW`，或是否应该路由到专门 skill。
5. 设计问题：只问会改变行为、范围、风险、测试、路由或实现许可的问题。
6. 提供选项：每个问题给 2-3 个互斥选项，并标出推荐答案、适用信号和代价。
7. 更新账本：用户选择、纠正或说“按推荐来”后，更新需求账本。
8. 继续推进：足够对齐就进入行为、评审、测试或阶段闸门；不够就继续追问或 probe。
...
- 验证锚点；
- 剩余风险；
- 下一步：按自适应路由推荐 `FAST_PATH`、`ccdawn-evaluation`、`ccdawn-bug-review`、`ccdawn-pr-review`、`ccdawn-planning` 或暂停。
