...
选定 owner 后，仅当收益高于成本才检查会话：`NONE / DISCOVER / PEER_CONTEXT_REVIEW / DISPATCH_READ_ONLY / DISPATCH_DISJOINT_WRITE / COORDINATE_OVERLAP`。相关 Agent 可只读了解进度；消息由 `ccdawn-thread-coordination` 按新增决策价值去重、聚合，无 finding 不发。owner 保留主线与集成。
