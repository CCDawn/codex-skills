...
- `HIGH/MEDIUM` 默认 0 个必须回答问题；`LOW` 且误解会返工时必须主动讨论并追问，一次集中提出 2-4 个相关问题并给出推荐默认；`BLOCKED` 只问当前不可替代的阻塞问题。
...
- `LOW`：存在多个合理行为结果且错判会返工；先读证据或做只读 probe，再进入讨论式意图收敛，未稳定前不执行写入。
...
- 同项目会话冲突、暂停/恢复：`ccdawn-thread-coordination`。
...
- 多会话冲突路由 `ccdawn-thread-coordination`；确认 `PAUSED` 后再写，完成后通知恢复。
