---
name: ccdawn-brt
description: "BRT workflow: Behavior, Review, Test。中文优先的需求对齐和流程入口闸门，用来主动揣测用户真实意图，给出有判断价值的候选意图、推荐方案和高信号选择题，让用户通过选择或确认完成对齐，再形成行为契约、证据化多角度自审结论、测试锚点和下一阶段路由。适用于模糊需求、默认行为、状态流转、权限安全、记忆/恢复、长任务、多步计划、阻塞恢复、验收标准、agent 工作流、promotion/apply/rollback、持久化与公共 API 行为。"
---
...
- 直接实现只能用于用户明确选择、单点小改、可逆、可验证且不改变核心行为的任务。
- 改代码前必须完成必要对齐；除非用户明确要求跳过。

## 流程路由

BRT 是 CCDawn 流程入口，只负责需求对齐和实现前闸门。需求对齐完成后不要默认直接实现，而是询问用户是否进入下一阶段。
...
6. 继续推进：足够对齐就进入行为、评审、测试或阶段闸门；不够就继续追问或 probe。
...
如果任何视角是 NEEDS_CLARIFICATION 或 NEEDS_CHANGE，不要进入下一阶段；先追问、调整行为契约或缩小范围。ACCEPT_RISK 必须说明被接受的剩余风险。
...
- 验证锚点；
- 剩余风险；
- 下一步：默认询问是否进入 `ccdawn-planning` 制定方案。
...
直接实现选项必须同时满足：用户明确选择、影响范围可枚举、无迁移/删除/权限/发布风险、可快速验证、失败可回滚。不满足时默认进入 `ccdawn-planning`。
