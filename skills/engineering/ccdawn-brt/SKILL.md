...
- 推荐意图、路由、方案或执行顺序时，必须形成轻量 `意图理由`（内部等价 `Intent Rationale`）：用户信号 / 我的推断 / 推荐理由 / 错判代价；`SILENT` / `FAST_PATH` 可只内部维护。
...
- ALIGN：意图可能偏差，优先输出 `对齐握手`；只有真实分叉时才给候选意图和少量高信号问题。
...
1. 读上下文：先限定上下文边界（内部字段 `Context Boundary`），再看相关代码、测试、文档、日志、配置或历史决策；memory 和全局规范只在续接、项目规则或历史决策会影响结论时读取。
...
6. 形成 `意图理由`：用户信号、我的推断、推荐理由、错判代价。
7. 形成 `对齐握手`：用户意图镜像、agent 行动预告、非目标、成功证据、纠偏点和默认推进。
8. 建立路由契约：负责人、模式、下一步产出、允许动作、成功证据、停止条件。
9. 写入前建立执行契约：目标、期望结果、允许动作、排除范围、成功证据、恢复信号。
10. 做 Wrong-Edit Guard：识别 owning surface、预计文件、保护边界和已有改动。
11. 自评流程重量：`FAST_PATH / COMPACT_FLOW / FULL_FLOW`，或路由到更具体的 skill。
12. 只问会改变行为、范围、风险、测试、路由或实现许可的问题；能默认推进的点用假设和纠偏点承接。
13. 足够对齐就执行、路由或进入阶段闸门；不够就继续追问或 probe。
