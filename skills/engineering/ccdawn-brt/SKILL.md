...
- 进入规划前必须先完成阶段顺序：意图对齐 -> 相关部分审查 -> 规划。相关部分审查只覆盖会改变方案的代码、测试、文档、配置、日志、历史决策、owning surface 和相邻风险，不做无目标全仓体检。
...
7. 形成 `对齐握手` 和 `意图锁定`：用户意图镜像、agent 行动预告、非目标、成功证据、纠偏点和默认推进。
8. 审查相关部分：只读检查会改变方案的 owning surface、相邻行为、测试、文档、配置、日志、历史决策和已有用户/agent 改动；输出关键证据、风险和保护边界。
9. 建立路由契约：负责人、模式、下一步产出、允许动作、成功证据、停止条件。
10. 写入前建立执行契约：目标、期望结果、允许动作、排除范围、成功证据、恢复信号。
11. 做 Wrong-Edit Guard：识别 owning surface、预计文件、保护边界和已有改动。
12. 自评流程重量：`FAST_PATH / COMPACT_FLOW / FULL_FLOW`，或路由到更具体的 skill。
13. 只问会改变行为、范围、风险、测试、路由或实现许可的问题；能默认推进的点用假设和纠偏点承接。
14. 足够对齐且相关审查足以支撑方案时，才执行、路由到 planning 或进入阶段闸门；不够就继续追问或 probe。
...
对齐后再做评审和测试，不要把它们放在意图发现前面；规划前必须至少完成相关部分审查，规划中如果发现新影响面，可以继续补充审查并修订方案。
