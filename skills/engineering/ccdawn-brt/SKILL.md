...
```text
user message -> intent guess -> response depth -> skill routing -> action shape
```
...
BRT 是 CCDawn 流程入口，只负责需求对齐和实现前闸门。需求对齐完成后先判定执行模式；如果用户目标已经包含执行许可，就推进到下一个自然闸门，而不是每阶段都重新询问。不要为了流程完整强迫进入规划、任务拆分或 BDD/TDD。
...
3. 判断意图置信度：HIGH / MEDIUM / LOW / BLOCKED。
4. 建立 Execution Contract：明确 Target、Desired Outcome、Allowed Actions、Out of Scope、Success Evidence、Recovery Signal。
5. 做 Wrong-Edit Guard：识别 owning surface、预计文件、保护边界和已有改动。
6. 主动揣测：只有 LOW 或 BLOCKED 时才输出候选意图或问题；HIGH/MEDIUM 内部完成揣测。
7. 自评流程重量：判断是否 `FAST_PATH / COMPACT_FLOW / FULL_FLOW`，或是否应该路由到专门 skill。
8. 设计问题：只问会改变行为、范围、风险、测试、路由或实现许可的问题。
9. 提供选项：阻塞时 1 个问题；需求对齐时最多 2-3 个互斥选项，并标出推荐答案、适用信号和代价。
10. 更新账本：用户选择、纠正或说“按推荐来”后，更新需求账本。
11. 继续推进：足够对齐就执行、路由或进入阶段闸门；不够就继续追问或 probe。
