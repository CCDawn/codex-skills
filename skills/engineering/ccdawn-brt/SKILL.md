...
选定 owner 后，仅当独立 lane 收益高于协调成本才检查会话。结果：`NONE / DISCOVER / DISPATCH_READ_ONLY / DISPATCH_DISJOINT_WRITE / COORDINATE_OVERLAP`。优先复用空闲会话；简单任务、关键路径、同文件或忙碌会话选 `NONE`。owner 保留主线与集成。
...
文件多或“可能存在库”不是触发条件。普通 CRUD、样式、小 bug、机械改动和已有模式直接实施。复用结论稳定时回原 owner，不强制 Planning。
...
首次写入、scope 扩大或合并前，有 registry 才静默运行 `preflight`。`CLEAR/PEERS_NO_OVERLAP` 继续；`OVERLAP` 进入 Silent Conflict Triage，不发消息。优先缩 scope、做非冲突工作、复用结果或等待；仅真实写冲突阻塞关键路径且需对方行动时协调。无 registry 不初始化 Memory、claim 或 coordination。
