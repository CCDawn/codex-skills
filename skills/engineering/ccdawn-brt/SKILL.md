...
选定 owner 后，收益高于协调成本才检查会话。结果：`NONE / DISCOVER / PEER_CONTEXT_REVIEW / DISPATCH_READ_ONLY / DISPATCH_DISJOINT_WRITE / COORDINATE_OVERLAP`。相关 Agent 处理同一 surface/接口时，可只读了解进度并给一次高价值建议；无 finding 不发消息。owner 保留主线与集成。
