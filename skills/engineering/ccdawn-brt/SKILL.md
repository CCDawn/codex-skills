...
7. 建立 Route Contract：Owner、Mode、Next Output、Allowed Action、Success Evidence、Stop Condition。
8. 写入前建立 Execution Contract：Target、Desired Outcome、Allowed Actions、Out of Scope、Success Evidence、Recovery Signal。
9. 做 Wrong-Edit Guard：识别 owning surface、预计文件、保护边界和已有改动。
10. 自评流程重量：`FAST_PATH / COMPACT_FLOW / FULL_FLOW`，或路由到更具体的 skill。
11. 只问会改变行为、范围、风险、测试、路由或实现许可的问题。
12. 足够对齐就执行、路由或进入阶段闸门；不够就继续追问或 probe。
