...
- 推荐意图、路由、方案或执行顺序时，必须形成轻量 `Intent Rationale`：用户信号 / 我的推断 / 推荐理由 / 错判代价；`SILENT` / `FAST_PATH` 可只内部维护。
- 对齐优先采用 `One-Turn Alignment`：一次性说清用户意图镜像、agent 行动预告、非目标、成功证据和纠偏点，减少用户输入次数。
...
- 每次用户可见回复的正文末尾都要给 `Next Action`：用一句话推荐下一步动作，除非自然闸门需要用户取舍才给少量选项。
...
- ALIGN：意图可能偏差，优先输出 `One-Turn Alignment`；只有真实分叉时才给候选意图和少量高信号问题。
...
`One-Turn Alignment` 用于减少用户输入消息次数。agent 先承担理解、默认选择和下一步行动预告，用户只在理解偏了或高风险取舍处纠偏。
...
- 用户已确认推荐时，写入 Intent Lock / Route Contract 并推进，不再重复要求确认。
...
6. 形成 `Intent Rationale`：用户信号、我的推断、推荐理由、错判代价。
7. 形成 `One-Turn Alignment`：用户意图镜像、agent 行动预告、非目标、成功证据、纠偏点和默认推进。
8. 建立 Route Contract：Owner、Mode、Next Output、Allowed Action、Success Evidence、Stop Condition。
9. 写入前建立 Execution Contract：Target、Desired Outcome、Allowed Actions、Out of Scope、Success Evidence、Recovery Signal。
...
- 开发规范、需求规约、官方文档一致性、项目 agent 规则、代码质量门禁、ADR/决策记录：只在对应外部 skill 已安装时作为 owner；未安装时用 `ccdawn-brt` Intent Lock、`ccdawn-planning`、本地 `AGENTS.md`/标准文档扫描和 `ccdawn-pr-review`。
...
- 默认 0 个必须回答问题；阻塞时 1 个；需求对齐时最多 2-3 个高信号选项，但优先打包进 `One-Turn Alignment` 的默认推进和纠偏点；高风险任务最多 4 个。
...
关键字段：用户原话、Intent Bundle、候选意图、已确认意图、Intent Rationale、用户目标、可观察结果、范围边界、操作边界、Route Contract、Execution Contract、保护边界、失败路径、验证证据、显式假设和剩余风险。
...
`Next Action` 是每次回复的尾句要求，作用是让用户知道下一步该做什么。它必须是用户可见正文的最后一行，格式保持轻量：
...
- 如果当前消息已经包含执行许可且无自然闸门，`Next Action` 写 agent 将继续推进的具体动作，不等待用户再确认。
- 如果存在阻塞、高风险远程写入、发布、合并、删除、迁移、权限动作或真实取舍，`Next Action` 才给 2-3 个选项，并标出推荐项。
- 不为了满足尾句而新增范围；下一步建议必须来自 Route Contract、Ordered Fix Queue、验证结果或当前结论。
- 若系统或平台要求附加引用、机器可读块或安全声明，把 `Next Action` 放在用户可见正文的最后，系统附录可以在其后。

每轮 BRT 简短收口：

- 已对齐的真实意图；
- 已排除的误解；
- 推荐理由 / Intent Rationale；
- 对齐握手 / One-Turn Alignment；
- 当前行为契约；
- 执行契约和保护边界；
- 自审结论；
- 验证锚点；
