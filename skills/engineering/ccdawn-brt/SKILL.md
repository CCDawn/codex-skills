...
如果这些字段里有一项未知且会影响行动，路由尚未完成；先 probe 或问一个关键问题。需要输出时用一行压缩说明：

```text
路由判断: Owner = ...；Mode = ...；Next Output = ...；Success Evidence = ...
```

不要为了展示流程把所有候选路由都列出来。常见路由实践场景见 `references/routing-practice.md`，只有路由犹豫、用户要求审查路由或需要校准新场景时读取。
...
6. 建立 Route Contract：明确 Owner、Mode、Next Output、Allowed Action、Success Evidence、Stop Condition。
7. 做 Wrong-Edit Guard：识别 owning surface、预计文件、保护边界和已有改动。
8. 主动揣测：只有 LOW 或 BLOCKED 时才输出候选意图或问题；HIGH/MEDIUM 内部完成揣测。
9. 自评流程重量：判断是否 `FAST_PATH / COMPACT_FLOW / FULL_FLOW`，或是否应该路由到专门 skill。
10. 设计问题：只问会改变行为、范围、风险、测试、路由或实现许可的问题。
11. 提供选项：阻塞时 1 个问题；需求对齐时最多 2-3 个互斥选项，并标出推荐答案、适用信号和代价。
12. 更新账本：用户选择、纠正或说“按推荐来”后，更新需求账本和 Intent Bundle。
13. 继续推进：足够对齐就执行、路由或进入阶段闸门；不够就继续追问或 probe。
