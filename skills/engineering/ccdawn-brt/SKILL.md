...
首次写入、scope 扩大或合并前，仅当项目已有 registry 时静默运行 `agent_coordination.py <root> preflight --agent-id <id> --scope <scope> --json`。`CLEAR/PEERS_NO_OVERLAP` 本阶段直接继续；`OVERLAP` 才路由协调。无 registry 不初始化 Memory、claim 或 coordination。
