...
4. 主动揣测：只有 LOW 或 BLOCKED 时才输出候选意图或问题；HIGH/MEDIUM 内部完成揣测。
5. 自评流程重量：判断是否 `FAST_PATH / COMPACT_FLOW / FULL_FLOW`，或是否应该路由到专门 skill。
6. 设计问题：只问会改变行为、范围、风险、测试、路由或实现许可的问题。
7. 提供选项：阻塞时 1 个问题；需求对齐时最多 2-3 个互斥选项，并标出推荐答案、适用信号和代价。
8. 更新账本：用户选择、纠正或说“按推荐来”后，更新需求账本。
9. 继续推进：足够对齐就执行、路由或进入阶段闸门；不够就继续追问或 probe。
