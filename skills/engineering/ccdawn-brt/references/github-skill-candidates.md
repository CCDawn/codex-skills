...
| React/Next.js performance patterns | `vercel-react-best-practices` | vercel-labs/agent-skills | Vercel-maintained React/Next guidance is more specific for data fetching, bundle, rendering, image, and component performance issues. | `ccdawn-performance-engineering` + frontend owner |
...
| Performance profiling | `performance-optimization`; `react-component-performance` | addyosmani/agent-skills; Dimillian/Skills | Performance work should measure before optimizing and avoid speculative complexity. | `ccdawn-performance-engineering` |
...
| Migration/deprecation | `deprecation-and-migration`; `codebase-migrate` | addyosmani/agent-skills; ComposioHQ/awesome-codex-skills | Removing/replacing systems requires consumer, rollout, and rollback evidence. | `ccdawn-planning` with TASK_GRAPH when needed |
...
| Incremental multi-file implementation | `incremental-implementation` | addyosmani/agent-skills | Thin vertical slices reduce wrong edits when a change spans multiple files but does not need full BDD/TDD ceremony. | `ccdawn-planning` TASK_GRAPH with SIMPLE tasks |
