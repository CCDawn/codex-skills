...
| Debugging recovery workflow | `debugging-and-error-recovery` | addyosmani/agent-skills | Reproduce, localize, reduce, fix, and guard is a stronger route for messy failures than generic implementation. | `systematic-debugging` + `root-cause-tracing` |
...
| Production logs and agent traces | `datadog-logs`; `langsmith-fetch` | ComposioHQ/awesome-codex-skills | Logs and LLM traces need source-linked evidence, privacy boundaries, and digestable failure patterns. | `systematic-debugging` + observability fallback |
