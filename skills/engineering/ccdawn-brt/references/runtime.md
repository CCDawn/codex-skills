...
Superpowers entrypoints are disabled by default and preserved only as reversible fallbacks. BRT mode and the current Route Contract decide whether brainstorming, planning, worktree isolation, strict TDD, subagents, review loops, or branch finishing have concrete value; restoring one method never restores its downstream chain. Verification evidence remains mandatory but proportional to the claim and risk.
...
When a subtask needs TDD, use the compact `ccdawn-bdd-tdd-development` owner. Reuse a real failing test or stable reproduction as RED, run the narrow test to GREEN, and defer broad suites to integration risk or closeout.
...
`ccdawn-bug-review` is the primary bug/failure owner; `root-cause-tracing` is added only when the source is hidden deep in a call chain.
...
- Prefer the most specific CCDawn owner. Route AI/ML research direction and multi-round evidence synthesis to `ccdawn-ai-research-loop`, concrete score/benchmark lanes to `ccdawn-score-loop`, important research claims to `ccdawn-research-rigor-review`, complex feature reuse research to `ccdawn-feature-reuse-research`, whole-project/codebase/architecture/technical-debt review to `ccdawn-project-review`, bug/failure work to `ccdawn-bug-review`, deep source tracing to `root-cause-tracing`, and PR/diff/review-feedback work to `ccdawn-pr-review`. If a later stage detects the wrong owner, route back to BRT or hand off directly.
...
- If the main value is diagnosing a failing behavior, route to `ccdawn-bug-review`; do not create a ledger unless recovery or cross-session handoff needs one.
