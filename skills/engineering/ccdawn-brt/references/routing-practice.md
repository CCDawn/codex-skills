...
| "另一个会话也在改 / 文件冲突 / 让对方暂停 / 完成后通知恢复" | `ccdawn-thread-coordination` | `COMPACT_FLOW` | confirmed owner plus pause/resume handshake | target thread verified, `PAUSED` received before conflicting write, resolution verified, resume message read back | target unknown, ownership disputed, no pause acknowledgement, thread tools unavailable |
