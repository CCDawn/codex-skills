...
| "多文件实现 / 小步推进 / 自动继续 / 不要太繁琐" | installed `incremental-implementation`; fallback compact planning + task splitting with SIMPLE tasks | `COMPACT_FLOW` | ordered thin slices or direct implementation route | each slice has owning files, verification, rollback/recovery signal | slice crosses API/data/security/release risk or requires BDD/TDD |
...
| "代码太复杂 / 简化 / 清理技术债 / 架构腐化" | installed `code-simplification` or `brooks-lint`; fallback `ccdawn-project-review` then ordered fix queue | `COMPACT_FLOW` | simplification candidates or decay-risk review | behavior-preservation boundary, affected files, tests/structural checks, deferred risks | requested cleanup would change behavior, scope too broad, no safety tests |
| "前端页面 / 组件 / UI 改版 / 响应式 / 视觉不像产品" | installed `frontend-ui-engineering`, `frontend-design`, or `vercel-react-best-practices`; fallback current project UI patterns + browser verification | `COMPACT_FLOW` or `FULL_FLOW` | UI intent lock, implementation plan/diff, or design review | user-visible outcome, design-system boundary, responsive states, accessibility/visual evidence named | no runnable UI, design target unclear, user asks for pure backend |
| "审 UI / 设计系统合规 / accessibility / responsive check" | installed `frontend-design-review`; fallback `ccdawn-pr-review` or `ccdawn-project-review` + browser evidence | `COMPACT_FLOW` | design-quality findings and ordered fix queue | screenshot/DOM/responsive/accessibility evidence tied to each finding | no UI target, no visual/runtime access, review turns into implementation |
...
| "Kaggle / 竞赛 / leaderboard / empirical research pipeline" | installed Kaggle/AERS candidate; fallback `ccdawn-competition-research-lifecycle` + `ccdawn-score-loop` | `FULL_FLOW` | lifecycle stage contract, data/metric/baseline/score route | competition brief, metric, data path, baseline, submission/experiment evidence identified | missing competition source, rules/private data risk, score metric unknown |
...
| "处理 PR 评论 / address comments / reviewer 要求" | installed `gh-address-comments`; fallback `receiving-code-review` | `COMPACT_FLOW` | comment-by-comment disposition and safe changes | each comment mapped to accept/reject/clarify with evidence and tests | PR/comment source missing, review asks for unsafe scope |
...
| "Sentry 报错 / 线上错误 / production stack trace" | installed `sentry-triage`; fallback `systematic-debugging` + `root-cause-tracing` | `COMPACT_FLOW` | stack-to-source diagnosis and guard/fix | issue/event inspected or pasted, stack frames mapped, repro or proof step found | no stack/event data, secrets/PII risk, cannot access source |
| "Datadog 日志 / LangSmith trace / agent eval / LLM 调用异常" | installed `datadog-logs` or `langsmith-fetch`; fallback `systematic-debugging` + observability route | `COMPACT_FLOW` | log/trace digest and root-cause route | queried or pasted evidence bounded by time/source, sensitive fields avoided, failing pattern linked to code/prompt | no access, privacy risk, time range/project unknown |
...
| "迁移 / 废弃旧系统 / 删除旧 API / 大规模替换" | installed `deprecation-and-migration` or `codebase-migrate`; fallback planning + task splitting | `FULL_FLOW` | migration plan, phases, rollback, user impact | old/new systems, consumers, rollout, validation, rollback evidence | destructive action, unknown consumers, no rollback |
...
| "OpenAI/Codex/API 文档 / PDF/Word/PPT/Excel / 生成图片" | installed system skill (`openai-docs`, `pdf`, `documents`, `presentations`, `spreadsheets`, `imagegen`) | `FAST_PATH` or `COMPACT_FLOW` | artifact, official-doc answer, or file operation result | official/current source or rendered/validated artifact evidence | system skill unavailable, file missing, high-stakes decision needs more context |
