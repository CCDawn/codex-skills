...
| Superpowers process skill broadly says "always / every project / any feature" | BRT Superpowers compatibility gate before invoking the process skill | current BRT mode | direct execution or only the risk-relevant method | selected method changes correctness/误改 risk; skipped methods have no material value | user explicitly requests strict workflow or current mode is FULL_FLOW |
| "用 TDD / 测试优先 / 这个回归要稳" | `ccdawn-bdd-tdd-development` compact profile for the selected risky subtask; do not dual-load Superpowers TDD | `COMPACT_FLOW` | minimal RED -> GREEN evidence chain | existing failure reused or one focused test fails correctly, narrow test passes, related regression check passes | behavior expectation unclear, RED is environmental, broader scope required |
