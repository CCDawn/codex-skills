...
- After alignment on nontrivial project work, proactively perform one bounded same-project thread discovery when native thread tools are available and reusable context, complementary expertise, or disjoint lanes could materially help. Discovery sends no messages. Route a valuable team to `ccdawn-multi-agent-orchestration`; route one-off advice or conflict to `ccdawn-thread-coordination`. Skip discovery for simple work or unavailable thread capabilities.
