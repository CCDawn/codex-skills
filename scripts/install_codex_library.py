...
- For feature work, silently classify code-efficiency risk as `FAST / CHECK / PROFILE`: keep obvious local inefficiency with the current owner even when it is described as a discovered problem; use `ccdawn-bug-review` only for an observed failure, correctness regression, or unresolved root cause, and load `ccdawn-performance-engineering` only for a measurable target/regression or material hot-path, scale, concurrency, or resource risk. Do not benchmark routine changes.
