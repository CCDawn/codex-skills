...
- `~/.codex/skills/<skill-name>`：给 Codex 运行时直接加载的 live skill 目录

如果你明确需要，也还保留可选的第二层目录：

- `~/.agents/skills/<skill-name>`：给某些应用环境下的本地 skill 目录使用
...
- 正式安装到 `~/.codex/skills/<skill-name>` 时，运行 `python scripts/install_codex_library.py`，使用真实目录复制。
...
1. 先看 `~/.codex/skills/<skill-name>` 里的 live 安装副本，不要只看仓库副本。
2. 检查是否还存在额外的 `~/.agents/skills/<skill-name>` 副本；同一个 skill 两边都装时，Codex 里可能出现重复条目。
